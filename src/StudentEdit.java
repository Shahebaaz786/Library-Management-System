...
class StudentEdit extends JFrame implements ActionListener,KeyListener
{
...
lblName=new JLabel("Update Student Information");
...
txtFName.setText(rs.getString(5));
txtMName.setText(rs.getString(6));
txtLName.setText(rs.getString(7));
txtAdd.setText(rs.getString(9));
txtTal.setText(rs.getString(12));
txtDist.setText(rs.getString(11));
txtMob.setText(rs.getString(13));
             } 
}
catch(Exception e)
{
...
if(ae.getSource()==btnUpdate)
{
 try
	 ...
	 Class.forName("oracle.jdbc.driver.OracleDriver");
	 Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","root");
PreparedStatement ps=con.prepareStatement("update Student set Dept=?,FName=?,MName=?,LName=?,Village=?,District=?,Taluca=?,Contact=? where Id=?");
